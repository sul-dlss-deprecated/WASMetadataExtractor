...
/**
 * It is the main class that receives the user arguments and calls the 
 * engine 
 * @author aalsum
 *
 */
public class WASMetadataExtractorApp {
	...
	public static void main(String[] args) {
		...
		if (wasInputFileList.length() > 0) {
			...
			try {
				...
				e.printStackTrace();
				System.err.println("There is a problem in reading "
						+ wasInputFileList);
				return;
			}
		...
		} else if (wasOutputFormat.length() == 0
				|| "TXT".equalsIgnoreCase(wasOutputFormat)) {
		...
		if (wasConfigFile != null && wasConfigFile.length() > 0) {
			engine = new MetadataExtractorEngine(fileList, wasConfigFile,
					writer);
		} else {
	...
	private static ArrayList<File> readFileListFromDirectory(
		...
		for (File file : listOfFiles) {
			if (file.isDirectory()) {
				fileList.addAll(readFileListFromDirectory(file.getPath()
						+ file.getName()));
			} else {
				fileList.add(file);
			}
	...
	private static ArrayList<File> readFileListFromFile(String listFileName)
			throws IOException {
	...
	private static void addCommandLineOptions() {
		...
		options.addOption(OptionBuilder
				...
				.hasArg()
				.withDescription(
						"An input file that has a list of ARC/WARC files to be processed.")
				.create("i"));
		options.addOption(OptionBuilder
				...
				.withLongOpt("inputdir").create("d"));

		options.addOption(OptionBuilder.withArgName("file").hasArg()
				.withDescription("The output file.").withLongOpt("outputFile")
				.create("o"));

		options.addOption(OptionBuilder.withArgName("XML|TXT").hasArg()
		...
		options.addOption(OptionBuilder
				...
				.hasArg()
				.withDescription(
						"YAML configuration file to extract the metadata.")
				.withLongOpt("config").create("c"));
		options.addOption(OptionBuilder
				.withArgName("collectionId")
				.hasArg()
				.withDescription(
						"Collection id as it should appear in metadata files. It shouldn't have space.")
				.withLongOpt("collectionId").create());
		options.addOption(OptionBuilder
				.withArgName("crawlId")
				.hasArg()
				.withDescription(
						"The crawl id as it should appear in metadata file.")
				.withLongOpt("crawlId").create());
	}
